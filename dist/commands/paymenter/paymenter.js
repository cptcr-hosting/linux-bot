...
const paymenter_api_1 = __importDefault(require("paymenter-api"));
// @ts-ignore: 2835
const apiKey_1 = __importDefault(require("../../models/apiKey"));
// @ts-ignore: 2835
const returnFile_1 = require("../../helpers/returnFile");
// Example config import:
// @ts-ignore: 2835
const config_1 = __importDefault(require("../../helpers/config"));
const shopUrl = config_1.default.billing;
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .addChoices({ name: "Multiple Accounts", value: "multiple_accounts" }, { name: "Name Violation", value: "name_violation" }, { name: "Custom", value: "custom" })
    .setRequired(true))
    .addStringOption((o) => o
    .setName("priority")
    ...
    .setRequired(true))
    .addNumberOption((o) => o.setName("user-id").setDescription("User ID").setRequired(true))
    .addStringOption((o) => o.setName("custom-title").setDescription("Custom title"))
    .addStringOption((o) => o.setName("custom-description").setDescription("Custom description")))
    .addSubcommand((sub) => sub.setName("ticket-get-all").setDescription("Get all tickets (admin)"))
    .addSubcommand((sub) => sub
    ...
    .addStringOption((o) => o.setName("ticket-id").setDescription("Ticket ID").setRequired(true)))
    .addSubcommand((sub) => sub.setName("invoice-get-all").setDescription("Get all invoices (admin)"))
    .addSubcommand((sub) => sub
    .setName("invoice-get")
    ...
    .addChoices({ name: "Manual", value: "Manual" }, { name: "PayPal", value: "PayPal" }, { name: "Stripe", value: "Stripe" }, { name: "Stripe Subscriptions", value: "Stripe Subscriptions" }, { name: "VivaWallet", value: "VivaWallet" })
    .setRequired(true))))
    // =============================
    ...
    .addChoices({ name: "High", value: "high" }, { name: "Medium", value: "medium" }, { name: "Low", value: "low" })
    .setRequired(true)))
    .addSubcommand((sub) => sub.setName("ticket-get-all").setDescription("Get all user tickets"))
    .addSubcommand((sub) => sub
    .setName("ticket-get")
    ...
    .addStringOption((o) => o.setName("ticket-id").setDescription("Ticket ID").setRequired(true)))
    .addSubcommand((sub) => sub.setName("invoice-get-all").setDescription("Get all invoices (user)"))
    .addSubcommand((sub) => sub
    .setName("invoice-get")
    .setDescription("Get invoice by ID (user)")
    .addStringOption((o) => o.setName("invoice-id").setDescription("Invoice ID").setRequired(true))));
async function run({ interaction }) {
        ...
        if (command === "ticket-create") {
            ...
            const prebuilts = {
                multiple_accounts: {
                    ...
                    description: "We have identified multiple accounts under the same user. Contact support.",
                },
                name_violation: {
                    title: "Urgent: Name violates TOS",
                    description: "Your username violates TOS. Please change within 24h or risk termination.",
                },
                custom: {
                    title: customTitle,
            ...
            try {
                ...
                const res = await paymenter_api_1.default.Admin.Ticket.create({
                    panel: shopUrl,
                    apikey: apiKey,
                ...
                const embed = new discord_js_1.EmbedBuilder()
                    ...
                    .addFields({ name: "Priority", value: priority, inline: true }, {
                    name: "User",
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const res = await paymenter_api_1.default.Admin.Ticket.getAll({ panel: shopUrl, apikey: apiKey });
                if (!res.ok)
                ...
                const file = await (0, returnFile_1.createFile)(JSON.stringify(ticketObj, null, 2), "admin_tickets.json");
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const r = await paymenter_api_1.default.Admin.Ticket.getAllMessages({
                    panel: shopUrl,
                    apikey: apiKey,
                    ticketId,
                });
                ...
                const file = await (0, returnFile_1.createFile)(JSON.stringify(js.data, null, 2), "admin_ticket_messages.json");
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const res = await paymenter_api_1.default.Admin.Ticket.getById({
                    panel: shopUrl,
                    apikey: apiKey,
                    ticketId,
                });
                ...
                if (!ticketObj?.id)
                    throw new Error("No valid ticket data.");
                const msgRes = await paymenter_api_1.default.Admin.Ticket.getAllMessages({
                    panel: shopUrl,
                    apikey: apiKey,
                    ticketId,
                });
                ...
                if (!msgJson?.success)
                    throw new Error(msgJson?.message || "No messages.");
                const file = await (0, returnFile_1.createFile)(JSON.stringify(msgJson.data, null, 2), "admin_ticket_messages.json");
                const embed = new discord_js_1.EmbedBuilder()
                    .setTitle(`Admin Ticket #${ticketObj.id}`)
                    .setURL(`${shopUrl}/admin/tickets/${ticketObj.id}`)
                    .setDescription("Ticket data + messages.")
                    .addFields({ name: "Title", value: ticketObj.title || "No Title" }, { name: "Status", value: ticketObj.status || "Unknown", inline: true }, { name: "Priority", value: ticketObj.priority || "Not Set", inline: true }, { name: "Created", value: ticketObj.created_at || "Unknown" }, { name: "Updated", value: ticketObj.updated_at || "Unknown" });
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const r = await paymenter_api_1.default.Admin.Invoice.getAll({ panel: shopUrl, apikey: apiKey });
                if (!r.ok)
                    throw new Error(`HTTP ${r.status}`);
                ...
                const file = await (0, returnFile_1.createFile)(JSON.stringify({ invoices: j.data, metadata: j.metadata }, null, 2), "admin_invoices.json");
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const res = await paymenter_api_1.default.Admin.Invoice.getById({
                    panel: shopUrl,
                    apikey: apiKey,
                    invoiceId,
                });
                ...
                if (!inv)
                    throw new Error("No invoice object.");
                const file = await (0, returnFile_1.createFile)(JSON.stringify(inv, null, 2), `admin_invoice_${invoiceId}.json`);
                const embed = new discord_js_1.EmbedBuilder()
                    .setTitle(`Invoice #${inv.id}`)
                    .setURL(`${shopUrl}/admin/invoices/${inv.id}`)
                    .setDescription("Invoice details.")
                    .addFields({ name: "Status", value: inv.status || "Unknown", inline: true }, { name: "User ID", value: String(inv.user_id), inline: true }, { name: "Paid At", value: inv.paid_at || "N/A" }, { name: "Due Date", value: inv.due_date || "N/A" });
                return interaction.editReply({ files: [file], embeds: [embed] });
            }
            catch (error) {
                return interaction.editReply({ content: `Error: ${error.message}` });
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const r = await paymenter_api_1.default.Admin.Invoice.pay({
                    panel: shopUrl,
                    apikey: apiKey,
                    invoiceId,
    ...
    // ========== USER GROUP ==========
        ...
        if (command === "ticket-create") {
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const r = await paymenter_api_1.default.Client.Ticket.create({
                    panel: shopUrl,
                    apikey: apiKey,
                    title,
                    description,
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const r = await paymenter_api_1.default.Client.Ticket.getAll({ panel: shopUrl, apikey: apiKey });
                if (!r.ok)
                    throw new Error(`HTTP ${r.status}`);
                const j = await r.json();
                if (!j?.success) {
                ...
                const f = await (0, returnFile_1.createFile)(JSON.stringify(t, null, 2), "user_tickets.json");
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const r = await paymenter_api_1.default.Client.Ticket.getById({
                    panel: shopUrl,
                    apikey: apiKey,
                    ticketId,
                });
                ...
                const embed = new discord_js_1.EmbedBuilder()
                    ...
                    .addFields({ name: "Title", value: ticketObj.title || "No Title" }, { name: "Status", value: ticketObj.status || "Unknown", inline: true }, { name: "Priority", value: ticketObj.priority || "N/A", inline: true }, { name: "Created At", value: ticketObj.created_at || "N/A" });
                return interaction.editReply({ embeds: [embed] });
            }
            catch (error) {
                return interaction.editReply({ content: `Error: ${error.message}` });
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const r = await paymenter_api_1.default.Client.Ticket.getMessages({
                    panel: shopUrl,
                    apikey: apiKey,
                    ticketId,
                });
                ...
                const msgs = j.data;
                const f = await (0, returnFile_1.createFile)(JSON.stringify(msgs, null, 2), `ticket_${ticketId}_messages.json`);
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const r = await paymenter_api_1.default.Client.Ticket.deleteById({
                    ticketId,
                    apikey: apiKey,
                    panel: shopUrl,
                });
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const r = await paymenter_api_1.default.Client.Invoice.getAll({ panel: shopUrl, apikey: apiKey });
                if (!r.ok)
                    throw new Error(`HTTP ${r.status}`);
                const j = await r.json();
                if (!j?.success) {
                ...
                const f = await (0, returnFile_1.createFile)(JSON.stringify({ invoices: j.data, metadata: j.metadata }, null, 2), "user_invoices.json");
                return interaction.editReply({ files: [f] });
            }
            catch (error) {
                return interaction.editReply({ content: `Error: ${error.message}` });
            ...
            try {
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const r = await paymenter_api_1.default.Client.Invoice.getById({
                    panel: shopUrl,
                    apikey: apiKey,
                    invoiceId,
                });
                ...
                const f = await (0, returnFile_1.createFile)(JSON.stringify(inv, null, 2), `user_invoice_${invoiceId}.json`);
                const embed = new discord_js_1.EmbedBuilder()
                    .setTitle(`Invoice #${inv.id}`)
                    .setURL(`${shopUrl}/invoice/${inv.id}`)
                    .addFields({ name: "Status", value: inv.status || "Unknown" }, { name: "Paid At", value: inv.paid_at || "N/A" }, { name: "Due Date", value: inv.due_date || "N/A" }, { name: "Created At", value: inv.created_at || "N/A" });
